...
if __name__ == "__main__":
    ...
    start = time.time()
    result, size, initial_best_size, length, branch_num = find_max_flexiclique(G, tau)
    end = time.time()
    ...
    output_filename = f"{tau}_EBA_sort.dat"
    output_path = os.path.join(output_dir, output_filename)

    with open(output_path, 'w') as output_file:
        output_file.write(f"branch_num: {branch_num}\n")
        output_file.write(f"{tau}-clique: initial best size: {initial_best_size}\n")
        output_file.write(f"{tau}-clique, running time :  {elapsed_time}\n")
